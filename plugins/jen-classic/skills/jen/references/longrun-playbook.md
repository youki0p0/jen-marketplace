...
## Classic long-horizon notes

- 1サイクル=タスク1つを厳守。まとめ処理はドリフトの温床。
- セッションローテーション: 8サイクル毎、またはコンテキスト圧迫を感じたら
  handoff更新 → 新セッション(/model opus) → 「.jen/handoff.md を読んで再開」。
- 委譲規律は同じ: PMO(opus)自身が実装を始めたらdrift。
- コストチェックポイント: 5サイクルごとに「opusで処理した工程のうちsonnet以下へ
