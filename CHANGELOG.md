...
### 同リリース系列に含まれる公開済み変更（2026-07-03 に v3.1.0 として main へ反映済み）
- **jen-architect を opus → fable に昇格** — 設計はトークン軽量かつ下流への影響が最大のため、
  上流に最高知能を置く（Fable 使用箇所は PMO・architect・deep-solver の3箇所に）
- **Claude Design（claude.ai/design）をデザイン正本化** — jen-frontend は `/design-sync` で
  同期されたライブラリに忠実に実装し、デザインを発明しない。jen-ux-critic が一致を検収
