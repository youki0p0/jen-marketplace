...
から各workerへ委譲していた。この構造では解釈が劣化した時点で原文がどこにも残らず
復旧できない（伝言ゲーム問題）。v3.8で役割を分離した。

### 発見した構造的欠陥（この変更のきっかけ）
- `tools:` は**許可リスト**であり、`Agent`（サブエージェント起動ツール。旧称Task）を
  持つJen agentが**1つも存在しなかった**。つまり `jen-pmo` は委譲権限を持たず、
  「PMOとして専門agentへ委譲する」という自身の定義を実行不能だった。
  これが実質メインセッションがPMOを代行していた根本原因。
- 同じ理由で「builder/architect/debugger は自分専用の scout を起動してよい」
  （model-tiering.md / routing-policy.md）も**実現不能な記述**だった。撤回。

### 変更
- **`jen-pmo` に `Agent` を付与し、オーケストレーションの唯一の担当にした**。
  モード定義・ルーティング表・全プロトコルを `agents/jen-pmo.md` へ集約（単一の正）
- **`skills/jen/SKILL.md` を伝言役の定義だけに縮小**（183行 → 65行）。
  許可: 原文保全 / jen-pmo起動 / 結果の転送 / Human Gate仲介 / プラグインルート解決。
  禁止: タスク分解・受入条件作成・worker への直接委譲・自分でコードを触ること・
  **ユーザー入力の言い換え**
- **`.jen/inbox.md`（追記専用の原文ログ）を新設**。メインセッションが一字一句
  そのまま追記し、jen-pmo は委譲promptに `[IN-xxx]` の原文を丸ごと引用する。
  workerが迷ったら要約ではなく原文を読む。伝言ゲーム対策の本体
- 新規リファレンス `references/relay-protocol.md`
- **jen-pmo は毎回まっさらな状態で起動される**ことを明記し、`.jen/` からの
  再アンカリングを起動時の必須手順にした（サブエージェントはステートレス）
- 行動監査に「伝言の忠実性」「メインセッションの越権」の2軸を追加

### 構造的制約として明記したこと
- **サブエージェントは `AskUserQuestion` を必ず剥奪される** → jen-pmo は
  ユーザーに直接質問できない。Human Gate では質問文を出力して停止し、
  メインセッションが提示・回答を逐語で inbox へ追記して再起動する。
  これがメインセッションに残る唯一の対話的責務
- **⚠️ 未検証の前提**: インタラクティブセッション（fork mode ON が既定）では
  サブエージェントが background 実行になり、公式ドキュメントの
  「background subagentが保持する組み込みツール」一覧に `Agent` が含まれない。
  一方で「既定で3階層まで入れ子にできる」とも記載され競合している。
  **導入後1回、jen-pmo が scout へ委譲できるか確認すること**。できない場合の
  回避策（`CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`）を relay-protocol.md に記載

### 副次的な訂正
- agent frontmatter の調査結果を反映: `memory`（user|project|local）と
  `background` は**公式にサポートされている**（v3.7.2で「未確認」に含めたのは誤り）。
  未確認なのは `effort` / `isolation` / `color`
- メインセッションのモデル推奨を変更: PMOを兼ねなくなったため `/model fable` は
  必須ではなくコスト削減の余地になった。ただし伝言役に必要なのは指示追従の
  忠実性そのものなので極端に安いモデルは避ける
